...
export async function updateBudget(data: { name: string; budgeted: number }): Promise<{ success?: BudgetCategory, error?: any }> {
  ...
  try {
    const updatedBudget = await updateBudgetLimitInService(validatedData.data.name, validatedData.data.budgeted);
    if (!updatedBudget) {
